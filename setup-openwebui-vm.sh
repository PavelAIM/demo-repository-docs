...
wait_for_vm() {
    ...
    while [ $attempt -le $max_attempts ]; do
        if multipass exec $VM_NAME -- echo "VM is ready" &>/dev/null; then
            print_status "VM is ready for connections"
            break
        fi
        
        print_status "Waiting for VM... (attempt $attempt/$max_attempts)"
        sleep 5
        ((attempt++))
    done
