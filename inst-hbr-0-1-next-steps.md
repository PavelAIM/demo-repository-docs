...
#### **Вариант A: Простой (docker без sudo)**
```bash
...
Спасибо за уточнение! Действительно важно разделять эти концепции. В корпоративной среде стоит стремиться к rootless mode, но для быстрого старта можно использовать группу docker.
